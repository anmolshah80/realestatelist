...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!isAdmin) {
      ...
      mainSql = `
        SELECT
          id, address, bedrooms, bathrooms, price, "yearBuilt", "listingDataSource", longitude, latitude, "livingArea", "lotSize", currency, "dateSoldString", country, "livingAreaUnits", description, "daysOnZillow", "brokerageName", "propertyTypeDimension", photos, url, "isFeatured", interior, tag, "lastSoldPrice", "priceHistory", "createdAt", "updatedAt"
        FROM "PropertyListing"
        ${whereClause}
        ORDER BY "createdAt" DESC
        OFFSET $${paramIndex++} LIMIT $${paramIndex++}
